package com.comit.video.glview;
...
import com.comit.video.gles.WindowSurface;
...
public class RenderThread extends Thread {
	...
	void queueEvent(Runnable r) {
		...
		Message.obtain(mRenderHandler, MSG_EVENT, r).sendToTarget();
	}
	...
	public static class RenderHandler extends Handler {
		...
		@NonNull
		private final List<Runnable> mPendingEvent = new ArrayList<>();
...
//				throw new IllegalStateException("mWindowSurface is null.");
		...
		private void onEvent(Runnable r) {
			if (!canRender()) {
				mPendingEvent.add(r);
				return;
			}
		...
		private void executePendingEvent() {
			if (!mPendingEvent.isEmpty()) {
				final Runnable[] events = mPendingEvent.toArray(new Runnable[0]);
				mPendingEvent.clear();
